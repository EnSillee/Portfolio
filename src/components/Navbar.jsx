...
const Navbar = ({isToggleOn, handleClick}) => {
  ...
  return (
    <div className=" jua fixed w-full h-[80px] flex justify-between items-center px-4 bg-[#4485F5] text-white">
      <Link to="home" className=' flex items-center cursor-pointer ml-0 lg:ml-32' smooth={true} duration={300}>
        <img src={Logo} alt="Logo" style={{ width: '65px' }} />
        <span className=' text-white text-lg lg:text-2xl'>E n S i l l e e</span>
      </Link>

      {/* menu */}
      <ul className="hidden md:flex lg:mr-32 text-xl">
        <Link smooth={true} duration={500} to='home' className=' px-4 lg:text-2xl cursor-pointer'>Home</Link>
        <Link smooth={true} duration={500} to='about' className=' px-4 lg:text-2xl cursor-pointer'>About</Link>
        <Link smooth={true} duration={500} to='skills' className=' px-4 lg:text-2xl cursor-pointer'>Skills</Link>
        <Link smooth={true} duration={500} to='projects' className=' px-4 lg:text-2xl cursor-pointer'>Projects</Link>
        <Link smooth={true} duration={500} to='contact' className=' px-4 lg:text-2xl cursor-pointer'>Contact</Link>
      </ul>
      ...
      {/* Mobile menu */}
      <ul className={isToggleOn? 'absolute top-0 left-0 w-full h-screen bg-[#4485F5] flex flex-col justify-center items-center' : 'hidden'}>
